...
import { Container, Row, Col, Card, Button, Table, Badge, Alert, Tab, Tabs, Modal, Form, Pagination, Toast, ToastContainer, Spinner } from 'react-bootstrap';
import { FaUsers, FaShoppingBag, FaChartLine, FaPlus, FaEdit, FaTrash, FaTimes, FaUpload, FaImage } from 'react-icons/fa';
import { useAuth } from '../contexts/AuthContext';
import LoadingSpinner from '../components/LoadingSpinner';
import { api } from '../services/api';
import { formatPrice, formatDate, formatOrderStatus, getStatusColor, getImageUrl } from '../utils/format';
...
const AdminDashboard = () => {
  ...
  const [showProductToast, setShowProductToast] = useState(false);
  const [productToastMessage, setProductToastMessage] = useState('');
  ...
  useEffect(() => {
    ...
    switch (activeTab) {
      case 'users':
        ...
        break;
      case 'orders':
        // Orders already loaded on mount
        break;
      case 'products':
        if (products.length === 0) loadProducts();
        break;
      case 'discounts':
        // Always reload discounts when tab is selected
        loadDiscounts();
        break;
      case 'categories':
        loadCategories();
      ...
      case 'analytics':
        ...
        if (!advancedDashboardData.analytics) {
  ...
  const loadDiscounts = async () => {
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        console.log('📦 Discounts loaded:', discountData.length, 'items');
        console.log('📦 Discount codes:', discountData.map(d => d.code));
  ...
  const loadCategories = async () => {
    try {
      ...
      try {
        const fallbackResponse = await api.get('/products/categories');
        if (fallbackResponse.data.success) {
          setCategories(fallbackResponse.data.data.stats || []);
        }
      } catch (fallbackError) {
  ...
  const handleCreateCategory = async () => {
    try {
      ...
      if (!categoryForm.displayName.trim()) errors.displayName = 'Display name is required';
      ...
      if (response.data.success) {
        ...
        loadCategories();
        loadCategoryListForProducts(); // Refresh dropdown list
  ...
  // Load category list for product dropdown
  const loadCategoryListForProducts = async () => {
    try {
      const response = await api.get('/categories/list');
      if (response.data.success) {
        setCategoryListForProducts(response.data.data || []);
      }
    } catch (error) {
      console.error('Failed to load category list:', error);
      // Fallback to hardcoded list
      setCategoryListForProducts([
  ...
  // Load inventory (low stock products)
  const loadInventory = async () => {
    try {
      setInventoryLoading(true);
      // Load all products and filter for low stock
      const response = await api.get('/admin/products', {
        params: { limit: 100 }
      });
      if (response.data.success) {
        const allProducts = response.data.data;
        // Filter products with low stock (totalStock <= 10)
        const lowStock = allProducts.filter(p => p.totalStock <= 10);
        setLowStockProducts(lowStock);
        // Also update products state
  ...
  const handleUpdateStock = async (productId) => {
    try {
      ...
      const updatedVariants = product.variants.map((variant, index) => ({
        ...variant,
        stock: stockUpdateForm[`variant_${index}`] !== undefined 
          ? parseInt(stockUpdateForm[`variant_${index}`]) 
          : variant.stock
      }));
  ...
  const handleOpenStockModal = (product) => {
    ...
    product.variants?.forEach((variant, index) => {
      initialForm[`variant_${index}`] = variant.stock || 0;
    });
  ...
  const handleSyncStock = async () => {
    try {
      setInventoryLoading(true);
      const response = await api.post('/admin/products/sync-stock');
      if (response.data.success) {
        alert(`Stock synced successfully! Updated ${response.data.data.updatedCount} products.`);
        loadInventory();
        loadProducts();
      }
    } catch (error) {
      console.error('Failed to sync stock:', error);
      alert('Failed to sync stock');
    } finally {
      setInventoryLoading(false);
    }
  };
  ...
  const handleOpenProductModal = (product = null) => {
    ...
    loadCategoryListForProducts(); // Load categories for dropdown
  ...
  const handleImageUpload = async (index, files) => {
    ...
    try {
      ...
      if (response.data.success && response.data.data.length > 0) {
        ...
        setProductToastMessage('Image uploaded successfully!');
        setShowProductToast(true);
      }
    } catch (error) {
  ...
  const handleProductSubmit = async () => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        setProductToastMessage('Product updated successfully!');
        setShowProductToast(true);
      } else {
        // Create
        await api.post('/admin/products', productData);
        setProductToastMessage('Product created successfully!');
        setShowProductToast(true);
      }
  ...
  const handleDiscountSubmit = async () => {
    ...
    try {
      ...
      // Close modal first, then reload discounts to ensure UI updates properly
      handleCloseDiscountModal();
      
      // Force reload discounts list after a short delay to ensure state is clean
      setTimeout(async () => {
        await loadDiscounts();
      }, 100);
    } catch (error) {
  ...
  const handleTabSelect = (key) => {
    ...
    // Just set the active tab, useEffect will handle data loading
    setActiveTab(key);
  };
    ...
    <Container className="py-4">
      ...
      <ToastContainer position="top-end" className="p-3" style={{ zIndex: 9999 }}>
        <Toast 
          ...
          onClose={() => setShowProductToast(false)} 
          delay={3000} 
          autohide
          bg="success"
        >
          ...
          <Toast.Body className="text-white">{productToastMessage}</Toast.Body>
        </Toast>
      </ToastContainer>
      ...
      <Tabs activeKey={activeTab} onSelect={handleTabSelect}>
                ...
                <Card.Body>
                  {dashboardData.bestSellingProducts && dashboardData.bestSellingProducts.length > 0 ? (
                    <>
                      ...
                      <div className="mb-3">
                        <h6>Sales Chart</h6>
                        <div className="d-flex align-items-end" style={{ height: '150px', gap: '8px' }}>
                          {(() => {
                            const salesValues = dashboardData.bestSellingProducts.map(p => p.salesCount || 0);
                            const maxSales = Math.max(...salesValues);
                            const allZero = maxSales === 0;
                            
                            return dashboardData.bestSellingProducts.map((product, index) => {
                              const sales = product.salesCount || 0;
                              // Calculate bar height in pixels
                              const barHeight = allZero ? 15 : Math.max((sales / maxSales) * 120, 15);
                              
                              return (
                                <div key={index} className="d-flex flex-column align-items-center" style={{ flex: 1, height: '100%', justifyContent: 'flex-end' }}>
                                  <div 
                                    className={allZero ? "bg-secondary rounded-top" : "bg-success rounded-top"}
                                    style={{ 
                                      height: `${barHeight}px`,
                                      width: '100%',
                                      maxWidth: '60px',
                                    ...
                                    title={`${sales} sold`}
                                  ></div>
                                  <small className="mt-1 text-center text-truncate" style={{ fontSize: '9px', maxWidth: '60px' }}>
                                    {product.name.substring(0, 8)}...
                                  </small>
                                  <small className="text-muted" style={{ fontSize: '10px', fontWeight: 'bold' }}>
                                    {sales}
                                  </small>
                                </div>
                              );
        ...
        <Tab eventKey="analytics" title="Analytics">
          {/* Analytics Filter Controls - Clean Design */}
          <Card className="mb-4 border-0 shadow-sm">
            <Card.Body className="py-3">
              <Row className="align-items-center g-3">
                <Col xs="auto">
                  <span className="text-muted small fw-medium">Filter by:</span>
                </Col>
                <Col xs="auto">
                  <Form.Select
                    size="sm"
                ...
                <Col xs="auto">
                  <Button variant="primary" size="sm" onClick={loadAdvancedDashboard}>
                    Apply Filter
                  </Button>
                </Col>
              </Row>
                  ...
                  <Card.Body>
                    {advancedDashboardData.analytics.orders && advancedDashboardData.analytics.orders.length > 0 ? (
                      <>
                        ...
                        <div className="mb-3">
                          <h6>Orders Chart</h6>
                          <div className="d-flex align-items-end" style={{ height: '180px', gap: '12px', padding: '10px 0' }}>
                            {(() => {
                              const orderCounts = advancedDashboardData.analytics.orders.map(i => i.count);
                              const maxOrders = Math.max(...orderCounts);
                              const minOrders = Math.min(...orderCounts);
                              const allEqual = maxOrders === minOrders;
                              
                              return advancedDashboardData.analytics.orders.map((item, index) => {
                                // Calculate bar height in pixels
                                let barHeight;
                                if (allEqual) {
                                  barHeight = 120; // Same height if all equal
                                } else {
                                  barHeight = maxOrders > 0 ? Math.max((item.count / maxOrders) * 140, 30) : 30;
                                }
                                ...
                                return (
                                  <div key={index} className="d-flex flex-column align-items-center" style={{ flex: 1, height: '100%', justifyContent: 'flex-end' }}>
                                    ...
                                    ></div>
                                    <small className="mt-1 text-center" style={{ fontSize: '11px' }}>
                                      {item._id.year}
                                      {item._id.month && `-${String(item._id.month).padStart(2, '0')}`}
                                      {item._id.quarter && ` Q${item._id.quarter}`}
                                      {item._id.week && ` W${item._id.week}`}
                                    </small>
                                    <small className="text-muted" style={{ fontSize: '11px', fontWeight: 'bold' }}>
                                      {item.count}
                                    </small>
                                  </div>
                                );
                    ...
                    {advancedDashboardData.analytics.revenue && advancedDashboardData.analytics.revenue.length > 0 ? (
                      <>
                        ...
                        <div className="mb-3">
                          ...
                          <div className="d-flex align-items-end" style={{ height: '180px', gap: '12px', padding: '10px 0' }}>
                            {(() => {
                              const revenueValues = advancedDashboardData.analytics.revenue.map(i => i.revenue);
                              const maxRevenue = Math.max(...revenueValues);
                              const minRevenue = Math.min(...revenueValues);
                              const allEqual = maxRevenue === minRevenue;
                              
                              return advancedDashboardData.analytics.revenue.map((item, index) => {
                                // Calculate bar height in pixels
                                let barHeight;
                                if (allEqual) {
                                  barHeight = 120; // Same height if all equal
                                } else {
                                  barHeight = maxRevenue > 0 ? Math.max((item.revenue / maxRevenue) * 140, 30) : 30;
                                }
                    ...
                    {advancedDashboardData.analytics.products && advancedDashboardData.analytics.products.length > 0 ? (
                      <>
                        ...
                        <div className="mb-3">
                          ...
                          <div className="d-flex align-items-end" style={{ height: '180px', gap: '8px', padding: '10px 0' }}>
                            {(() => {
                              const products = advancedDashboardData.analytics.products.slice(0, 5);
                              const maxSold = Math.max(...products.map(i => i.totalSold));
                              
                              return products.map((item, index) => {
                                const barHeight = maxSold > 0 ? Math.max((item.totalSold / maxSold) * 140, 20) : 20;
        ...
        <Tab eventKey="discounts" title="Discounts">
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center">
              <h5 className="mb-0">Discount Management ({discounts.length} codes)</h5>
              <div>
                <Button variant="outline-secondary" size="sm" className="me-2" onClick={loadDiscounts} disabled={discountsLoading}>
                  {discountsLoading ? 'Loading...' : '🔄 Refresh'}
                </Button>
                    ...
                    {discounts.length === 0 ? (
                      <tr>
                        <td colSpan="8" className="text-center text-muted py-4">
                          No discount codes found. Click "Add Discount" to create one.
                        </td>
                      </tr>
                    ) : null}
            ...
            <Card.Body>
              {categoriesLoading ? (
                ...
                <>
                  <Row className="mb-4">
                    ...
                    <Col md={4}>
                      <Card className="text-center bg-success text-white">
                        <Card.Body>
                          <h3>{categories.reduce((sum, cat) => sum + (cat.productCount || cat.count || 0), 0)}</h3>
                    ...
                    <Col md={4}>
                      <Card className="text-center bg-info text-white">
                        <Card.Body>
                          <h3>{categories.filter(c => (c.productCount || c.count || 0) > 0).length}</h3>
                    ...
                    <tbody>
                      {categories.map((cat, index) => (
                        <tr key={cat._id || index}>
                          <td>{index + 1}</td>
                          <td>
                            <code>{cat.name || cat._id}</code>
                          </td>
                          <td>
                            <strong>{cat.displayName || (cat._id || cat.name || '').split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</strong>
                          </td>
                          <td>
                            <Badge bg="secondary">{cat.productCount || cat.count || 0} products</Badge>
                          </td>
                          <td>
                            <Badge bg={(cat.productCount || cat.count || 0) > 0 ? 'success' : 'warning'}>
                              {(cat.productCount || cat.count || 0) > 0 ? 'Active' : 'Empty'}
                            </Badge>
                          </td>
                          <td>
                            {(cat.productCount || cat.count || 0) === 0 && cat._id && (
                              <Button 
                                variant="outline-danger" 
                                size="sm"
                            ...
                            {(cat.productCount || cat.count || 0) > 0 && (
                      ...
                      {categories.length === 0 && (
                        <tr>
                          <td colSpan="6" className="text-center text-muted py-4">
                            No categories found. Click "Sync from Products" to import existing categories or "Add Category" to create new ones.
                          </td>
                        </tr>
                      )}
                    </tbody>
                  ...
                  <Alert variant="info">
                    <strong>💡 Tip:</strong> You can add new categories here, which will then be available in the product form dropdown.
                    Empty categories (0 products) can be deleted. Categories with products cannot be deleted to protect order history.
                  </Alert>
                </>
              )}
            </Card.Body>
        ...
        <Tab eventKey="inventory" title="Inventory">
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center">
              ...
              <div>
                <Button variant="outline-warning" size="sm" className="me-2" onClick={handleSyncStock} disabled={inventoryLoading}>
                  {inventoryLoading ? 'Syncing...' : '🔧 Sync Stock'}
                </Button>
                <Button variant="outline-secondary" size="sm" onClick={loadInventory} disabled={inventoryLoading}>
                ...
                <>
                  ...
                  <Row className="mb-4">
                    ...
                    <Col md={3}>
                      <Card className="text-center bg-success text-white">
                        <Card.Body>
                          <h3>{products.reduce((sum, p) => sum + (p.totalStock || 0), 0)}</h3>
                    ...
                    <Col md={3}>
                      <Card className="text-center bg-danger text-white">
                        <Card.Body>
                          <h3>{products.filter(p => p.totalStock === 0).length}</h3>
                  ...
                  <Table responsive striped hover>
                    ...
                    <tbody>
                      {products.map((product) => (
                        <tr key={product._id} className={product.totalStock <= 10 ? 'table-warning' : ''}>
                          <td>
                            <div className="d-flex align-items-center">
                              {product.images && product.images.length > 0 && (
                                <img 
                          ...
                          <td>
                            {product.variants?.map((v, i) => (
                              <div key={i}><small>{v.sku || 'N/A'}</small></div>
                            ))}
                          </td>
                          ...
                          <td>
                            {product.variants?.map((v, i) => (
                              <div key={i}>
                                <small>{v.name}: <strong>{v.stock}</strong></small>
                              </div>
                            ))}
                          </td>
                          <td>
                            <Badge bg={product.totalStock === 0 ? 'danger' : product.totalStock <= 10 ? 'warning' : 'success'}>
                              {product.totalStock}
                            </Badge>
                          </td>
                          <td>
      ...
      {/* Category Modal */}
      <Modal show={showCategoryModal} onHide={() => { setShowCategoryModal(false); setCategoryFormErrors({}); }}>
        ...
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3">
              <Form.Label>Category Name (slug) *</Form.Label>
              <Form.Control
                type="text"
              ...
              <Form.Text className="text-muted">
                Lowercase, hyphens instead of spaces (auto-formatted)
              </Form.Text>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                isInvalid={!!categoryFormErrors.displayName}
              />
              <Form.Control.Feedback type="invalid">
                {categoryFormErrors.displayName}
              </Form.Control.Feedback>
            </Form.Group>
            <Form.Group className="mb-3">
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => { setShowCategoryModal(false); setCategoryFormErrors({}); }}>
            Cancel
          </Button>
      ...
      <Modal show={showStockModal} onHide={() => setShowStockModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Update Stock - {selectedProductForStock?.name}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedProductForStock?.variants?.map((variant, index) => (
            <Form.Group key={index} className="mb-3">
              <Form.Label>
                {variant.name} (Current: {variant.stock})
              </Form.Label>
              <Form.Control
                type="number"
                min="0"
                value={stockUpdateForm[`variant_${index}`] || 0}
                onChange={(e) => setStockUpdateForm({
                  ...stockUpdateForm,
                  [`variant_${index}`]: e.target.value
                })}
              />
            </Form.Group>
          ))}
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={() => handleUpdateStock(selectedProductForStock?._id)}>
            Update Stock
          </Button>
        </Modal.Footer>
      </Modal>
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                >
                  ...
                  {categoryListForProducts.length > 0 ? (
                    categoryListForProducts.map(cat => (
          ...
          {productForm.images.map((image, index) => (
            <Card key={index} className="mb-3">
              <Card.Body>
                ...
                {image.url && (
                  <div className="mt-2 p-2 bg-light rounded d-flex align-items-center gap-3">
                    <img 
                      src={image.url} 
