...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      ...
      setFormData(prev => ({
        ...
        shippingAddress: formatFullAddress(defaultAddress)
      }));
  ...
  const handleAddressSelect = (e) => {
    ...
    if (addressId === 'new') {
      ...
      // Find selected address and fill form
      ...
      if (selectedAddress) {
        setFormData(prev => ({
          ...
          shippingAddress: formatFullAddress(selectedAddress)
        }));
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store order info before clearing cart
      const createdOrder = response.data.data.order || response.data.data;
      setOrderId(createdOrder._id);
      setOrderData({
        ...createdOrder,
        // Backup from form data in case backend doesn't return all fields
        formData: {
          email: formData.email,
          fullName: formData.fullName,
          phone: formData.phone,
  ...
  if (success && orderData) {
    const displayInfo = orderData.displayData || {};
    const orderInfo = orderData.formData || formData;
          ...
          <Row className="justify-content-center">
            <Col lg={8}>
                ...
                <Card.Body>
                  <Row className="mb-3">
                    ...
                    <Col sm={6} className="text-end">
                      <span className="text-primary">{orderData.orderNumber || orderId}</span>
                    </Col>
                  </Row>
                  <Row className="mb-3">
                    <Col sm={6} className="text-start">
                    ...
                    <Col sm={6} className="text-end">
                      {orderInfo.fullName || user?.fullName}
                    </Col>
                  </Row>
                  <Row className="mb-3">
                    <Col sm={6} className="text-start">
                  ...
                  <hr />
                  <Row className="mb-2">
                    <Col sm={6} className="text-start">Subtotal:</Col>
                    <Col sm={6} className="text-end">{formatPrice(displayInfo.subtotal || 0)}</Col>
                  </Row>
                  {displayInfo.discount > 0 && (
                    <Row className="mb-2 text-success">
                      <Col sm={6} className="text-start">Discount:</Col>
                      <Col sm={6} className="text-end">-{formatPrice(displayInfo.discount)}</Col>
                    </Row>
                  )}
                  {displayInfo.loyaltyPointsUsed > 0 && (
                    <Row className="mb-2 text-success">
                      <Col sm={6} className="text-start">Loyalty Points:</Col>
                      <Col sm={6} className="text-end">-{formatPrice(displayInfo.loyaltyPointsUsed)}</Col>
                    </Row>
                  )}
                  <Row className="mb-2">
                    <Col sm={6} className="text-start">Shipping:</Col>
                    <Col sm={6} className="text-end">{displayInfo.shipping > 0 ? formatPrice(displayInfo.shipping) : 'Free'}</Col>
                  </Row>
                  <Row className="mb-2">
                    <Col sm={6} className="text-start">Tax (10%):</Col>
                    <Col sm={6} className="text-end">{formatPrice(displayInfo.tax || 0)}</Col>
                  </Row>
                  <hr />
                  <Row className="mb-3">
                    <Col sm={6} className="text-start">
                    ...
                    <Col sm={6} className="text-end">
                      <strong className="text-primary fs-5">{formatPrice(displayInfo.total || 0)}</strong>
                    </Col>
                  </Row>
                  <Row>
                    <Col sm={6} className="text-start">
                    ...
                    <Col sm={6} className="text-end">
                      {paymentMethods.find(pm => pm.id === orderInfo.paymentMethod)?.name || orderInfo.paymentMethod}
                    </Col>
                  </Row>
                </Card.Body>
              </Card>
              ...
              {displayInfo.items && displayInfo.items.length > 0 && (
                <Card className="mb-4 shadow-sm">
                  <Card.Header>
                    <h5 className="mb-0">Order Items ({displayInfo.items.length})</h5>
                  </Card.Header>
                  <Card.Body>
                    {displayInfo.items.map((item, index) => (
                      <div key={index} className="d-flex justify-content-between align-items-center py-2 border-bottom">
                        <div>
                          ...
                          {item.variant && <Badge bg="secondary" className="ms-2">{item.variant.name}</Badge>}
