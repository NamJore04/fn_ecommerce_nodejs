...
import { Container, Row, Col, Card, Button, Badge, Alert, Tabs, Tab, Form, Modal, Spinner } from 'react-bootstrap';
import { useParams, useNavigate } from 'react-router-dom';
import { FaStar, FaStarHalfAlt, FaRegStar, FaShoppingCart, FaHeart, FaShare, FaMinus, FaPlus, FaCheck } from 'react-icons/fa';
...
const ProductDetail = () => {
  ...
  const DESCRIPTION_LIMIT = 300; // Characters before showing "See more"
  ...
  // Auto-fill comment form when user is logged in
  useEffect(() => {
    if (isAuthenticated && user) {
      setCommentForm(prev => ({
        ...prev,
        anonymousName: user.fullName || '',
  ...
  const loadReviews = async () => {
    try {
      ...
      // Always update product rating from reviews API
      // This ensures we have the latest calculated rating
      if (reviewData) {
        setProduct(prev => ({
          ...prev,
          averageRating: reviewData.averageRating ?? prev?.averageRating ?? 0,
          reviewCount: reviewData.ratingCount ?? prev?.reviewCount ?? 0
        }));
        console.log('⭐ Rating updated from reviews:', {
  ...
  const renderStars = (rating, interactive = false, onRatingChange = null) => {
    ...
    return (
      <div className="d-flex align-items-center">
        ...
        {/* Half star */}
        {hasHalfStar && !interactive && (
          <FaStarHalfAlt key="half" className="text-warning" />
        )}
        {/* Interactive half star position */}
        {hasHalfStar && interactive && (
          <FaStar
            key="half-interactive"
            className="text-warning cursor-pointer"
            onClick={onRatingChange ? () => onRatingChange(fullStars + 1) : undefined}
        ...
        {[...Array(emptyStars)].map((_, index) => (
          <FaRegStar
            key={`empty-${index}`}
            className={`text-muted ${interactive ? 'cursor-pointer' : ''}`}
            onClick={interactive && onRatingChange ? () => onRatingChange(fullStars + (hasHalfStar ? 2 : 1) + index) : undefined}
            style={{ cursor: interactive ? 'pointer' : 'default' }}
          />
        ))}
        {/* Show numeric rating */}
