...
export const AuthProvider = ({ children }) => {
  ...
  const socialLogin = async (provider) => {
    try {
      ...
      // 🔧 Dynamic OAuth URL based on environment
      // Construct base URL from API_URL (supports both Local and Docker)
      const API_URL = process.env.REACT_APP_API_URL || '/api';
      
      let baseUrl;
      if (API_URL.startsWith('/')) {
        // Docker environment: API_URL is relative ("/api")
        // Use current origin (http://localhost)
        baseUrl = window.location.origin;
      } else {
        // Local environment: API_URL is absolute ("http://localhost:5000/api")
        // Extract base URL by removing "/api"
      ...
      console.log('🔐 Social login redirect:', {
        provider,
        environment: API_URL.startsWith('/') ? 'Docker' : 'Local',
        baseUrl,
        oauthUrl
      });
