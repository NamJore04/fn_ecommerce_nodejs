...
export const CartProvider = ({ children }) => {
  ...
  const [loading, setLoading] = useState(false); // Only for initial cart load
  const [operationLoading, setOperationLoading] = useState({}); // Per-item loading state
  const [error, setError] = useState(null);
  const { isAuthenticated, user } = useAuth();

  // Helper to set loading for specific item
  const setItemLoading = (productId, variantId, isLoading) => {
    const key = `${productId}_${variantId || 'default'}`;
    setOperationLoading(prev => ({ ...prev, [key]: isLoading }));
  };

  // Check if specific item is loading
  const isItemLoading = (productId, variantId) => {
    const key = `${productId}_${variantId || 'default'}`;
  ...
  const loadCart = useCallback(async () => {
    try {
      ...
      if (isAuthenticated) {
        ...
        // Store entire cart object including discountCode, discountAmount, loyaltyPointsUsed
        if (cartData) {
          setCart({
          ...
          setCart({ items: [], discountCode: null, discountAmount: 0, loyaltyPointsUsed: 0 });
        }
      } else {
  ...
  const addToCart = async (product, variant, quantity = 1) => {
    ...
    try {
      setItemLoading(productId, variantId, true);
      setError(null);
      ...
      setCart(prev => ({
        ...prev,
        items: cartData?.items || [],
        discountCode: cartData?.discountCode || prev?.discountCode || null,
        discountAmount: cartData?.discount || prev?.discountAmount || 0,
        loyaltyPointsUsed: cartData?.loyaltyPointsUsed || prev?.loyaltyPointsUsed || 0
      }));
      ...
      setItemLoading(productId, variantId, false);
    }
  };
  ...
  const clearCart = async () => {
    try {
      ...
      // Reset cart to empty object structure
      setCart({ items: [], discountCode: null, discountAmount: 0, loyaltyPointsUsed: 0 });
  ...
  const removeDiscountCode = async () => {
    try {
      ...
      const cartData = response.data.data.cart;
      // Clear discount info while keeping other cart data
      setCart(prev => ({
        ...prev,
        items: cartData?.items || [],
        discountCode: null,
  ...
  const removeLoyaltyPoints = async () => {
    try {
      ...
      const cartData = response.data.data.cart;
      // Clear loyalty points while keeping other cart data
      setCart(prev => ({
        ...prev,
        items: cartData?.items || [],
        loyaltyPointsUsed: 0
