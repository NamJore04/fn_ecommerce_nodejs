...
export const SocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    // 🔧 Dynamic Socket URL detection for Local/Docker environments
    const getSocketUrl = () => {
      ...
      // Priority 2: Detect based on current location
      const currentPort = window.location.port;
      
      // Docker environment (port 80, 443, or empty)
      if (currentPort === '' || currentPort === '80' || currentPort === '443') {
        const socketUrl = window.location.origin;
        console.log('🐳 Docker detected → Socket URL:', socketUrl);
        return socketUrl;
      }
      
      // Local development (port 3000)
      console.log('💻 Local development → Socket URL: http://localhost:5000');
