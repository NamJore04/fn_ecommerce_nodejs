...
reviewSchema.statics.updateProductRating = async function(productId) {
  ...
  // Ensure productId is ObjectId for aggregate match
  const objectId = typeof productId === 'string' 
    ? new mongoose.Types.ObjectId(productId) 
    : productId;
  
  // Get average rating from reviews with rating
  ...
  // Get total review count (including comments without rating)
  ...
  console.log('📊 Rating stats for product', productId, ':', ratingStats, 'total:', totalCount);
...
// Static method to update only review count (for comments without rating)
reviewSchema.statics.updateProductReviewCount = async function(productId) {
  const Product = mongoose.model('Product');
