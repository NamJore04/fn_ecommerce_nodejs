...
const getProducts = asyncHandler(async (req, res) => {
  ...
  if (brand) {
    ...
    console.log('🏷️ Brand filter:', filter.brand);
  }
  ...
  if (minPrice || maxPrice) {
    ...
    console.log('💰 Price filter:', filter.basePrice);
  }

  if (search) {
    // Use regex for search if text index not available
    filter.$or = [
  ...
  if (rating) {
    ...
    console.log('⭐ Rating filter:', filter.averageRating);
  }
...
// @desc    Get featured products (top products by review count)
...
// @desc    Get best selling products (top 8 by sales count)
// @route   GET /api/products/bestsellers
// @access  Public
const getBestSellingProducts = asyncHandler(async (req, res) => {
  const { limit = 8 } = req.query;
  
  // Get products sorted by sales count (number of purchases)
  const products = await Product.find({ isActive: true })
    .sort({ salesCount: -1, totalStock: 1, createdAt: -1 })
    .limit(parseInt(limit));
