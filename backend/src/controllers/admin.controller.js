...
const getDashboard = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    revenue,
    recentOrders,
    bestSellingProductsAggregation
  ] = await Promise.all([
    User.countDocuments(),
    ...
    // Get best selling products from actual orders instead of salesCount field
    Order.aggregate([
      { $match: { createdAt: { $gte: startDate } } },
      { $unwind: '$items' },
      { $group: { 
  ...
  res.json({
    ...
    data: {
      ...
      recentOrders,
      bestSellingProducts: bestSellingProductsAggregation
    }
  });
});
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  // Recalculate totalStock from variants
  if (product.variants && product.variants.length > 0) {
    product.totalStock = product.variants.reduce((total, variant) => {
      return total + (variant.isActive !== false ? (variant.stock || 0) : 0);
    }, 0);
  }
...
const deleteUser = asyncHandler(async (req, res) => {
  ...
  console.log(`🗑️ Deleted ${deletedOrders.deletedCount} orders for user ${userId}`);
  
  // Delete user's cart
  const deletedCarts = await Cart.deleteMany({ user: userId });
  console.log(`🗑️ Deleted ${deletedCarts.deletedCount} carts for user ${userId}`);
  
  // Delete user's reviews
  const deletedReviews = await Review.deleteMany({ user: userId });
  console.log(`🗑️ Deleted ${deletedReviews.deletedCount} reviews for user ${userId}`);
  
  // Finally delete the user
  await User.findByIdAndDelete(userId);
  console.log(`🗑️ User ${user.email} deleted permanently`);
...
const uploadProductImage = asyncHandler(async (req, res) => {
  ...
  // Map uploaded files to relative URLs (works with proxy and direct access)
...
// @desc    Sync all products totalStock
// @route   POST /api/admin/products/sync-stock
// @access  Private/Admin
const syncProductsStock = asyncHandler(async (req, res) => {
  ...
  for (const product of products) {
    ...
    if (product.totalStock !== calculatedTotal) {
      product.totalStock = calculatedTotal;
      await product.save();
      updatedCount++;
      console.log(`📦 Fixed stock for ${product.name}: ${product.totalStock} -> ${calculatedTotal}`);
    }
  }
  
  res.json({
    success: true,
    message: `Stock synced for ${updatedCount} products`,
    data: { updatedCount, totalProducts: products.length }
  });
});
