...
const getReviews = asyncHandler(async (req, res) => {
  ...
  const averageRating = ratingReviews.length > 0 
    ? ratingReviews.reduce((sum, r) => sum + r.rating, 0) / ratingReviews.length 
    : 0;
  
  console.log('📊 Reviews for product', productId, '- Total:', reviews.length, ', With rating:', ratingReviews.length, ', Avg:', averageRating);
  ...
  res.status(200).json({
    ...
    data: {
      reviews,
      ratingCount: ratingReviews.length,
...
const createAnonymousComment = asyncHandler(async (req, res) => {
  ...
  // Update product review count (includes comments without rating)
  await Review.updateProductReviewCount(req.params.productId);
...
// @desc    Sync all product ratings (recalculate from reviews)
// @route   POST /api/reviews/sync-ratings
// @access  Private/Admin
const syncAllProductRatings = asyncHandler(async (req, res) => {
  ...
  console.log(`✅ Synced ratings for ${synced} products`);
  
  res.status(200).json({
    success: true,
    message: `Synced ratings for ${synced} products`
  });
});
