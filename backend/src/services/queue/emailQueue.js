...
const createEmailQueue = () => {
  ...
  if (!isRedisEnabled()) {
    console.log('⚠️ Redis disabled - email queue will use direct sending');
    redisEnabled = false;
    return null;
  }
  ...
  emailQueue.on('stalled', (job) => {
    ...
    console.error('❌ Failed to create email queue:', error.message);
    redisEnabled = false;
    return null;
  }
};
...
const addEmailJob = async (type, data, options = {}) => {
  ...
  if (!isRedisEnabled()) {
    console.log(`📧 Redis disabled - sending email directly (type: ${type})`);
    try {
      await sendEmailDirectly(type, data);
      console.log('✅ Email sent directly');
      return { id: 'direct-' + Date.now(), type };
    } catch (error) {
  ...
  try {
    ...
    if (!queue) {
      ...
      console.log('⚠️ Queue not available, sending email directly...');
      await sendEmailDirectly(type, data);
      console.log('✅ Email sent directly');
      return { id: 'direct-' + Date.now(), type };
    }
...
const sendEmailDirectly = async (type, data) => {
  switch (type) {
    ...
    case 'order-confirmation':
      return emailService.sendOrderConfirmationEmail(data.user, data.order);
    
    case 'password-reset':
      return emailService.sendPasswordResetEmail(data.user, data.resetToken);
    
    case 'order-status-update':
      return emailService.sendOrderStatusUpdateEmail(data.user, data.order, data.newStatus);
...
const processEmailQueue = (queue) => {
  ...
  queue.process('order-confirmation', async (job) => {
    ...
    await emailService.sendOrderConfirmationEmail(job.data.user, job.data.order);
  ...
  queue.process('password-reset', async (job) => {
    ...
    await emailService.sendPasswordResetEmail(job.data.user, job.data.resetToken);
    return { success: true, email: job.data.user.email };
  });
  ...
  queue.process('order-status-update', async (job) => {
    ...
    await emailService.sendOrderStatusUpdateEmail(job.data.user, job.data.order, job.data.newStatus);
    return { success: true, orderNumber: job.data.order.orderNumber };
  });
